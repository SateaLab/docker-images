...
docker exec -ti ff3e8b4e4c55 curl -s localhost:27657/status | jq .result | jq .sync_info 

docker exec -ti ff3e8b4e4c55 initiad --node http://localhost:27657 query bank balances $(echo "test1234" | initiad keys show "$moniker" -a) --output json

docker exec -ti ff3e8b4e4c55 initiad debug addr $(echo "test1234" |initiad keys show "$moniker" -a)
